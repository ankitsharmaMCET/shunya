...
if (Sys.info()["sysname"] == "Darwin") {
  ...
  GAMS_custom <- list(
    ...
    cmdline = "C:/GAMS/42/gams.exe energyRt.gms",
     lang = "GAMS",
    solver = "gurobi",
    export_format = "gdx"
    # solver = "cbc"
...
cap_1 <-ideea_data$generators_corrected_2020 
cap_1 <-summarise(
    select(
...
points_coord <- st_centroid(gis_mainland_sf) %>%
  st_coordinates() %>%
  as.data.frame() %>%
  cbind(data.frame(region = gis_mainland_sf$reg36)) %>%
  rename(lon = X, lat = Y)
...
if (nrow(network) > 0) {
  for (i in 1:nrow(network)) {
    trd <- newTrade(
      name = stringr::str_replace(network$trd_name[i], "TRD_", "TRN_"),
      # desc = network$lines_type[i],
      commodity = "ELC",
      routes = data.frame(
        src = c(network$region.x[i], network$region.y[i]),
...
```{r, eval=FALSE}
scen_base_1Dec <- interpolate(mod, name = "scen_base_1Dec")
summary(scen_base_1Dec)
...
scen_py <- write_sc(scen_sub,
                   ...
                   tmp.dir = file.path("tmp", "IDEEA_subset",
                                       "Pyomo_Cbc_barrier"))
